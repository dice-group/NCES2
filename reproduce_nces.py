...
if __name__ == '__main__':
    ...
    parser.add_argument('--emb_dim', type=int, default=40, help='KB embedding dimension')
    ...
    for kb in args.kbs:
        print()
        evaluate_nces(kb_name=kb, proj_dims=[64, 128, 256], emb_dim=args.emb_dim, args=nces_args, save_results=args.save_results, verbose=args.verbose) #kb_name, proj_dims, emb_dim, args, save_results=False, verbose=False
        print()
        
        if args.ensemble:
            print("*"*25 + " Evaluating ensemble model " + "*"*25)
            evaluate_ensemble(kb_name=kb, args=nces_args, emb_dim=args.emb_dim, save_results=args.save_results, verbose=args.verbose) # kb_name, args, emb_dim, save_results=False, verbose=False
            print("*"*25 + " Evaluating ensemble model " + "*"*25+"\n")
