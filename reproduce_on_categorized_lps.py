...
if __name__ == '__main__':
    ...
    for kb in args.kbs:
        print()
        evaluate_nces(kb_name=kb, num_inds=[32, 64, 128], args=nces_args, save_results=args.save_results, verbose=args.verbose)
        print()
        if args.ensemble:
            print("*"*25 + " Evaluating ensemble model " + "*"*25)
            evaluate_ensemble(kb_name=kb, args=nces_args, save_results=args.save_results, verbose=args.verbose)
            print("*"*25 + " Evaluating ensemble model " + "*"*25+"\n")
