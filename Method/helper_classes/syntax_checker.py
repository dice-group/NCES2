...
class SyntaxChecker:
    ...
    def __init__(self, knowledge_base, tokenizer):
    ...
    def preprocess(self, expression:str):    
        ...
        for i in range(len(dec_expression)):
            ...
            if filtered_atoms == []:
                if atom in ['∃', '∀'] and i < len(dec_expression)-2 and not dec_expression[i+1] in self.role_names \
                and not dec_expression[i+2] in self.role_names:
                    continue
                if atom in self.role_names and i < len(dec_expression)-1 and dec_expression[i+1] != '.':
                ...
                if not filtered_atoms[-1] in ['∃', '∀']:
                    if filtered_atoms[-1] in self.atomic_concept_names.union({'⊤', '⊥', ')'}):
                        filtered_atoms.extend(['⊔', '∃', atom])
                ...
                if i < len(dec_expression)-1:
                    if not dec_expression[i+1] in self.atomic_concept_names.union({'('}):
                ...
                if i < len(dec_expression)-2 and not dec_expression[i+1] in self.role_names \
                 and not dec_expression[i+2] in self.role_names:
                ...
                if i==len(dec_expression)-1:
                    ...
                    filtered_atoms.append(atom)
                    if (i<len(dec_expression)-1 and not dec_expression[i+1] in self.atomic_concept_names.union({'(', '⊤', '⊥', '∃', '∀', '¬'}))\
                    or i == len(dec_expression)-1:
                        filtered_atoms.append('⊤')
                        
            elif atom == '(':
                if not filtered_atoms[-1] in ['⊔', '⊓', '.']:
                    continue
                if i<len(dec_expression)-1 and dec_expression[i+1] in self.atomic_concept_names.union({'('}):
                    filtered_atoms.append(atom)
            elif atom == ')':
                if filtered_atoms.count('(') == filtered_atoms.count(')'):
                    continue
                if len(filtered_atoms) >= 2 and (filtered_atoms[-2] in {'⊔', '⊓'} or filtered_atoms[-1] == ')'):
                    filtered_atoms.append(atom)
...
class Evaluator:
    def __init__(self, knowledge_base):
        ...
        #self.concept_str_to_concept.update({c: knowledge_base.negation(c) for c in })
    ...
    def get_concept(self, expression_list:list):
        def disjunction_or_conjunction_concept_builder(atom_list):
            assert atom_list[0] == '(' and len(atom_list) >= 5, 'Incorrect expression form'
            if not '¬' in atom_list:
                if '⊓' in atom_list:
                    return self.knowledge_base.intersection([self.concept_str_to_concept[atom_list[i]] for i in list(range(len(atom_list)-1))[1::2]])
                return self.knowledge_base.union([self.concept_str_to_concept[atom_list[i]] for i in list(range(len(atom_list)-1))[1::2]])
            else:
                negations = [self.knowledge_base.negation(atom_list[i]) for i in \
                             range(2,len(atom_list)-2) if atom_list[i-1] == '¬']
                remaining_atoms = [atom_list[i] for i in \
                             range(1,len(atom_list)-1) if atom_list[i-1] != '¬' and atom_list[i] not in ['⊔', '⊓']]
                remaining_concepts = [self.concept_str_to_concept(c) for c in remaining_atoms]
                if '⊓' in atom_list:
                    return self.knowledge_base.intersection(negations+remaining_concepts)
                else:
                    return self.knowledge_base.union(negations+remaining_concepts)
        
        def get_block_concept(start_idx, expr):
            assert expr[start_idx] == '(', 'Missing parentheses, invalid block'
            out = []
            j = start_idx
            while out == [] or (out != [] and  out.count('(') != out.count(')')):
                out.append(expr[j])
                j += 1
            return j+1, disjunction_or_conjunction_concept_builder(out + [')'])
        i = 0
        current_expr = None
        join = None
        while i < len(expression_list):
            if expression_list[i] in ['⊔', '⊓']:
                ...
                if expression_list[i+3] in self.atomic_concept_names.union({'⊤', '⊥'}):
                    ...
                    j, filler = get_block_concept(i+3, expression_list)
                    prop = self.role_str_to_role[expression_list[i+1]]
                    ce = self.knowledge_base.existential_restriction(filler, prop) if expression_list[i] == '∃' else self.knowledge_base.universal_restriction(filler, prop)
                    if current_expr is None:
                        current_expr = ce
    ...
    def evaluate(self, list_atoms, pos_examples, neg_examples, verbose = True):
        all_individuals = set(self.knowledge_base.individuals())
        ce = self.get_concept(list_atoms)
        ...
        solution = self.renderer.render(ce)
        if verbose:
            print("Accuracy: {}%".format(100*acc))
            print("F1 score: {}%".format(100*f1))
        return solution, 100*acc, 100*f1