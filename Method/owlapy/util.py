...
class DNF:
    """This class contains functions to transform a Class Expression into Disjunctive Normal Form"""
    @singledispatchmethod
    def get_class_dnf(self, ce: OWLClassExpression, negated: bool = False) -> OWLClassExpression:
        """Convert a Class Expression to Disjunctive Normal Form. Operands will be sorted.
        ...
        Returns:
            Class Expression in Disjunctive Normal Form
            """
        raise NotImplementedError

    @get_class_dnf.register
    def _(self, ce: OWLClass):
        return ce

    @get_class_dnf.register
    def _(self, ce: OWLObjectIntersectionOf):
        ops = _sort_by_ordered_owl_object(ce.operands())
        if not any([isinstance(op, OWLObjectUnionOf) for op in ops]):
            return ce
        elif len(list(filter(lambda _: isinstance(_, OWLObjectUnionOf), ops))) == 1:
            union = list(filter(lambda _: isinstance(_, OWLObjectUnionOf), ops))[0]
            other = list(filter(lambda _: not isinstance(_, OWLObjectUnionOf), ops))
            return OWLObjectUnionOf([self.get_class_dnf(OWLObjectIntersectionOf([op, OWLObjectIntersectionOf(other)])) for op in union.operands()])
        else:
            union = list(filter(lambda _: isinstance(_, OWLObjectUnionOf), ops))
            other = list(filter(lambda _: not isinstance(_, OWLObjectUnionOf), ops))
            return self.get_class_dnf(OWLObjectIntersectionOf([union[0], self.get_class_dnf(OWLObjectIntersectionOf(union[1:]+other))]))

    @get_class_dnf.register
    def _(self, ce: OWLObjectUnionOf):
        ops = map(lambda _: self.get_class_dnf(_),
                  _sort_by_ordered_owl_object(ce.operands()))
        return OWLObjectUnionOf(ops)

    @get_class_dnf.register
    def _(self, ce: OWLObjectComplementOf):
        return self.get_class_dnf(ce.get_operand())

    @get_class_dnf.register
    def _(self, ce: OWLObjectSomeValuesFrom):
        filler = ce.get_filler()
        if isinstance(filler, OWLObjectUnionOf):
            return OWLObjectUnionOf([OWLObjectSomeValuesFrom(ce.get_property(), f) for f in filler.operands()])
        return OWLObjectSomeValuesFrom(ce.get_property(), filler)
    ...
    def _(self, ce: OWLObjectAllValuesFrom):
        filler = ce.get_filler()
        if isinstance(filler, OWLObjectUnionOf):
            filler_ = OWLObjectIntersectionOf([OWLObjectComplementOf(self.get_class_dnf(f)) for f in filler.operands()])
            return OWLObjectComplementOf(OWLObjectSomeValuesFrom(ce.get_property(), filler_))
        return ce
